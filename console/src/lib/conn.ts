...
const SIM_FALLBACK_FRAME: SimFrame = {
  t_us: 0,
  phase: 'WAIT',
  state: {
    pos_ned_m: [0, 0, 0],
...
/** Normalize a WS telemetry frame (§4.2 schema) with key tolerance. */
...
export function normalizeFleetSnapshot(
...
): { snapshot: FleetSnapshot; events: FleetEvent[]; auctions: { task: string; vehicle: string; bid_s: number; round: number; t: number; bidders: number }[] } | null {
  const body = asRec(unwrapEnvelope(raw)) ?? {}
  ...
  const auctions = (Array.isArray(body.auctions) ? body.auctions : [])
    .map((ar) => {
      ...
      return { task, vehicle, bid_s: num(r.bid_s, r.bid) ?? 0, round: num(r.round) ?? 0, t: num(r.t) ?? Date.now(), bidders: num(r.bidders) ?? 0 }
    })
