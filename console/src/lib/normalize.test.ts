...
// ---------------------------------------------------------------------------
// Sim-frame normalizer — wire fields + fallbacks to prev + SIM_FALLBACK_FRAME.
// ---------------------------------------------------------------------------

test('normalizeSimFrame: full wire frame → SimFrame', () => {
  const raw = {
    t_us: 12345,
    phase: 'RUN',
    state: {
    ...
    faults_active: [{ id: 'f1', type: 'motor_cut', motor: '1', since: 12340, ttl_s: 30 }],
    stats: { tick_p95_us: 200, sent: { hil_state: 60 }, recv: { actuator: 60 } },
  }
  const f = normalizeSimFrame(raw, null)
  assert.equal(f.t_us, 12345)
  assert.equal(f.phase, 'RUN')
  assert.deepEqual(f.state.pos_ned_m, [1, 2, -10])
  assert.equal(f.state.battery_pct, 87)
  assert.equal(f.sensors.gps_fix, 3)
  assert.equal(f.sensors.gps_sat, 14)
  assert.equal(f.faults_active.length, 1)
  assert.equal(f.faults_active[0].id, 'f1')
  // Wire value '1' (string) parses as a number → params.motor = 1 (number).
  // The normalizer prefers the parsed number; only non-numeric values stay strings.
  assert.equal(f.faults_active[0].params.motor, 1)
  assert.equal(f.stats.tick_p95_us, 200)
})

test('normalizeSimFrame: tolerates q (alias) for state.q_wxyz', () => {
  const raw = { state: { q: [0.7, 0, 0, 0.7] } }
  const f = normalizeSimFrame(raw, null)
  assert.deepEqual(f.state.q_wxyz, [0.7, 0, 0, 0.7])
})

test('normalizeSimFrame: empty raw + no prev → SIM_FALLBACK_FRAME shape', () => {
  const f = normalizeSimFrame(null, null)
  assert.equal(f.phase, SIM_FALLBACK_FRAME.phase)
  assert.deepEqual(f.state.q_wxyz, [1, 0, 0, 0])
  assert.equal(f.state.battery_pct, 100)
  assert.equal(f.sensors.gps_fix, 0)
})

test('normalizeSimFrame: prev carries forward when raw drops fields', () => {
  const prev: typeof SIM_FALLBACK_FRAME = {
    ...SIM_FALLBACK_FRAME,
    t_us: 1000,
    phase: 'RUN',
    state: { ...SIM_FALLBACK_FRAME.state, pos_ned_m: [5, 6, 7], battery_pct: 50 },
  }
  const f = normalizeSimFrame({}, prev)
  assert.equal(f.t_us, 1000, 't_us carried from prev')
  assert.equal(f.phase, 'RUN', 'phase carried from prev')
  assert.deepEqual(f.state.pos_ned_m, [5, 6, 7], 'pos carried from prev')
  assert.equal(f.state.battery_pct, 50, 'battery carried from prev')
})

// ---------------------------------------------------------------------------
// normalizeSimStatus — fields + tolerance.
// ---------------------------------------------------------------------------

test('normalizeSimStatus: full payload → SimStatus', () => {
  const raw = {
    phase: 'RUN',
    px4_connected: true,
    loop_closed: true,
    stats: { tick_p95_us: 250, rate_hz: 200 },
    t_us: 98765,
  }
  const s = normalizeSimStatus(raw, null)
  assert.equal(s.phase, 'RUN')
  assert.equal(s.px4_connected, true)
  assert.equal(s.loop_closed, true)
  assert.equal(s.tick_p95_us, 250)
  assert.equal(s.rate_hz, 200)
  assert.equal(s.t_us, 98765)
})

test('normalizeSimStatus: tolerates string bools + alt field names', () => {
  const raw = {
    run_phase: 'WAIT',
    connected: 'yes',
    lockstep: 'closed',
